...
export class BlackoutPoweredDeviceRig implements ResettablePuzzleComponent {
  ...
  constructor(options: BlackoutPoweredDeviceRigOptions) {
    ...
    this.controlledLaser = new LaserHazard({
    ...
    this.independentLaser = new LaserHazard({
    ...
    this.laserSystem = new LaserHazardSystem({
      ...
      hazards: [this.controlledLaser, this.independentLaser],
      requestRecovery: () => {
    ...
    this.laserJunction = new LaserJunctionDevice({
      ...
      hazards: [this.controlledLaser],
      mode: 'suppress-when-powered',
      powerMode: 'sustained',
    });

    this.devices = new ElectricalDeviceCollection(options.targetRegistry);
    ...
    try {
      ...
      this.devices.addSupplyLink(this.generator.id, this.light.id);
      this.devices.addSupplyLink(this.generator.id, this.platform.id);
      this.devices.addSupplyLink(this.generator.id, this.lift.id);
      this.devices.addSupplyLink(this.terminal.id, this.door.id);
