...
export class DroneBreakupPresentation {
  ...
  private readonly bursts: { age: number; x: number; y: number; z: number }[];
  ...
  constructor(capacity: number) {
    ...
    this.bursts = Array.from({ length: capacity }, () => ({ age: LIFETIME, x: 0, y: 0, z: 0 }));
  ...
  burst(slot: number, x: number, floorY: number, z: number): void {
  ...
  update(dt: number): void {
    ...
    for (const [slot, burst] of this.bursts.entries()) {
      if (burst.age >= LIFETIME) continue;
      burst.age += dt;
      const t = burst.age;
      if (t >= LIFETIME) continue;
      const shrink = 1 - THREE.MathUtils.smoothstep(t, .45, LIFETIME);
      for (let i = 0; i < FRAGMENTS; i++) {
        const angle = i * 2.39996 + slot * .71;
        const travel = .12 + t * (.65 + (i % 3) * .25);
        this.transform.position.set(burst.x + Math.cos(angle) * travel,
          burst.y + Math.max(.065, .18 + (1.8 + i * .12) * t - 5 * t * t),
      ...
      for (let i = 0; i < SPARKS; i++) {
        ...
        const radius = t * (2 + i % 3);
        const tail = Math.max(0, radius - .16 * (1 - t / .35));
  ...
  reset(): void {
    for (const burst of this.bursts) burst.age = LIFETIME;
