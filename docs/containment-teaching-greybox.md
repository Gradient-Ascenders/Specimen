...
| Chaining | Room 2 | A charged jump can catch an elevated sticky wall, continue around the ledge, and climb onto it. | Platform D faces a generous contaminated patch embedded in the east perimeter wall; the contamination wraps around the ledge's north fascia and top lip. |
...
- Room 2's contaminated ledge fascia and top strip expose
  `userData.textureRole = 'sticky-ledge-tile'`, allowing the wraparound route
  to receive a ledge-specific treatment without changing its gameplay tag.
...
- Confirm the Room 2 wall catch can move around the north side of the high
  ledge, climb its outer sticky face, and roll onto the sticky top strip.
